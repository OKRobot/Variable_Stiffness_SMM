...
plot(y1Vec(idx_rows(1:last_valid)), kVec(1:last_valid),'LineWidth',2);
xlabel('yi (meters)','fontsize',18);
ylabel('k(yi)','fontsize',18);
title('Stiffness as a function of yi','fontsize',21);
xlim([0.98*y1Vec(idx_rows(last_valid)), y1Vec(idx_rows(1))*1.02]);

p1 = [y1Vec(idx_rows(4)), kVec(4)];
p2 = [y1Vec(idx_rows(last_valid-2)), kVec(last_valid-2)];
